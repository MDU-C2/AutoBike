...
for episode = 1:20
    ...
    while(abs(currentState(1)) <= deathCartPos && abs(currentState(3))<=deathPoleAngle && actionNr < range)
        ...
        if mod(actionNr, 5) == 0
            action = 2*round(10*rand) - 10;
        else
            Eval = [currentState actions(1)];
            RunANN;
            Q1 = Q;
            
            Eval = [currentState actions(2)];
            RunANN;
            Q2 = Q;
            
            Eval = [currentState actions(3)];
            RunANN;
            Q3 = Q;
            
            Eval = [currentState actions(4)];
            RunANN;
            Q4 = Q;
        ...
        nextState = SimulatePendel(action, currentState(1), currentState(2), currentState(3), currentState(4));
        TrSet{1, episode} = [TrSet{1, episode} ; nextState];
        TrSet{2, episode} = [TrSet{2, episode}; action];
        currentState = nextState;
